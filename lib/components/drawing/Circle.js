...
function (_PureComponent) {
  ...
  function Circle(props, context) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getBounds", function () {
      return _this.state[_constants.CIRCLE].getBounds();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getCenter", function () {
      return _this.state[_constants.CIRCLE].getCenter();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getDraggable", function () {
      return _this.state[_constants.CIRCLE].getDraggable();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getEditable", function () {
      return _this.state[_constants.CIRCLE].getEditable();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getMap", function () {
      return _this.state[_constants.CIRCLE].getMap();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getRadius", function () {
      return _this.state[_constants.CIRCLE].getRadius();
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getVisible", function () {
      return _this.state[_constants.CIRCLE].getVisible();
    });

    var circle = new google.maps.Circle(props.options);
    _this.state = (_this$state = {}, _defineProperty(_this$state, _constants.CIRCLE, circle), _defineProperty(_this$state, "prevProps", (0, _MapChildHelper.construct)(_proptypes.CirclePropTypes, updaterMap, props, circle)), _this$state);
  ...
  _createClass(Circle, [{
    ...
    value: function getDerivedStateFromProps(props, state) {
      return (0, _MapChildHelper.getDerivedStateFromProps)(props, state, this.state[_constants.CIRCLE], eventMap, updaterMap);
    }
  }]);
