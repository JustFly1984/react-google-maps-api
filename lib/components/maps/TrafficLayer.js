...
function (_PureComponent) {
  ...
  function TrafficLayer(props, context) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getMap", function () {
      return _this.state[_constants.TRAFFIC_LAYER].getMap();
    });

    var trafficLayer = new google.maps.TrafficLayer(props.options);
    _this.state = (_this$state = {}, _defineProperty(_this$state, _constants.TRAFFIC_LAYER, trafficLayer), _defineProperty(_this$state, "prevProps", (0, _MapChildHelper.construct)(_proptypes.TrafficLayerPropTypes, updaterMap, props, trafficLayer)), _this$state);
  ...
  _createClass(TrafficLayer, [{
    ...
    value: function getDerivedStateFromProps(props, state) {
      return (0, _MapChildHelper.getDerivedStateFromProps)(props, state, this.state[_constants.TRAFFIC_LAYER], eventMap, updaterMap);
    }
  }]);
